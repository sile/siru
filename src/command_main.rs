...
fn print_detail<W: std::io::Write>(
    ...
    for (path, item) in &doc.show_items {
        ...
        if let Some(docs) = item.docs(&doc.json)? {
            let formatted_docs = add_rust_to_code_blocks(&docs);
            let increased_headings = increase_heading_levels(&formatted_docs);
            writeln!(writer, "{}\n", increased_headings)?;
        }
...
fn add_rust_to_code_blocks(text: &str) -> String {
...
fn increase_heading_levels(text: &str) -> String {
    text.lines()
        .map(|line| {
            if line.starts_with('#') {
